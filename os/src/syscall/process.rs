...
use crate::{
    ...
    task::{
        add_task, current_task, current_user_token, exit_current_and_run_next, get_current_processor_info,
        suspend_current_and_run_next, TaskInfo, current_processor_m_map, current_processor_m_unmap
    },
	timer::get_time_us,	
...
pub fn sys_task_info(ti: *const u8) -> isize {
    trace!("kernel:pid[{}] sys_task_info"
        , current_task().unwrap().pid.0);
    let token = current_user_token();
    let v = translated_byte_buffer(token, ti, 8);
    let ptr = v[0].as_ptr() as *mut TaskInfo;
